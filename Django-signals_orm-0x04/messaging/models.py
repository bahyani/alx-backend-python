...
class UnreadMessagesManager(models.Manager):
    """
    Custom manager to filter and retrieve unread messages for a specific user.
    Includes query optimizations using .only() to retrieve only necessary fields.
    """
    
    def unread_for_user(self, user, optimize=True):
        """
        Get all unread messages for a specific user.
        
        Args:
            user: User object to filter messages for
            optimize: Whether to use .only() for field optimization (default True)
        ...
        if optimize:
            ...
            queryset = queryset.only(
                'id', 'sender', 'content', 'timestamp', 'is_read', 'parent_message'
    ...
    def unread_count_for_user(self, user):
        ...
        Optimized to only count without fetching full objects.
    ...
    def unread_threads_for_user(self, user):
        """
        Get unread messages grouped by thread (conversation).
        
        Args:
            user: User object
            
        Returns:
            QuerySet of unread messages with thread information
        """
        return self.filter(
            receiver=user,
            is_read=False
    ...
    def mark_thread_as_read(self, thread_root, user):
        ...
        Args:
            thread_root: Root message of the thread
            user: User who is marking messages as read
        ...
        thread_messages = Message.objects.filter(
            models.Q(id=thread_root.id) | models.Q(parent_message=thread_root)
        ).filter(receiver=user, is_read=False)
        
        return thread_messages.update(is_read=True)
...
class Message(models.Model):
    ...
    def __str__(self):
        edited_marker = " (edited)" if self.edited else ""
        reply_marker = " (reply)" if self.parent_message else ""
        read_marker = "" if self.is_read else " [UNREAD]"
        return f"Message from {self.sender.username} to {self.receiver.username} at {self.timestamp}{edited_marker}{reply_marker}{read_marker}"
